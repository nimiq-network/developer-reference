...
Distinct Node types are used for different usecases and they differenciate from one another in the amount of data they need to download in order to join and synchronize with the network, and proporcionally the time it takes each Node type to establish consensus and be able to process and validate transactions.

### Full Nodes

Download the full blockchain thus requiring more storage. We expect this Node type to be used solely with NodeJS Clients although theoretically you could run it also in a Browser Client. 

### Light Nodes

Securely sync to almost full consensus by downloading just about 100 MB. 
